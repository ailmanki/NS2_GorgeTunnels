...
-- for showing beacon to location text
kTechDataShowBeaconToLocation = "beaconstolocation"
...
-- used to specify that only one entity should be processed at once.
kTechDataOneAtATime = "neverkeepprocessing"
...
function SetCachedTechData(techId, fieldName, data, force)
    ...
    if techData[fieldName] == nil or force then
...
function LookupTechData(techId, fieldName, default)
    ...
    if GetWarmupActive() and (fieldName == kTechDataUpgradeCost or fieldName == kTechDataCostKey) then
        return 0
    end
...
function GetRecycleAmount(techId)
